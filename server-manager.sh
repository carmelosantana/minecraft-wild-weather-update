...
# Wild Weather Update Plugin Server Manager
...
# Configuration
SERVER_DIR="server"
PLUGIN_NAME="wild-weather-update"
PLUGIN_JAR="target/${PLUGIN_NAME}-1.0.0-SNAPSHOT.jar"
...
online-mode=false
white-list=false
motd=Wild Weather Update Test Server
view-distance=10
...
EOF
    ...
    print_status "Note: Use '/weather help' for weather commands"
}
...
show_status() {
    ...
    # Check plugin status
    if [[ -f "$SERVER_DIR/plugins/${PLUGIN_NAME}-1.0.0-SNAPSHOT.jar" ]]; then
        print_success "Wild Weather Update plugin is installed"
    else
        print_warning "Wild Weather Update plugin is not installed"
    fi
}
...
case "${1:-help}" in
    ...
    players)
        ...
        ;;
    help|*)
        echo "Wild Weather Update Plugin Server Manager"
        echo ""
